...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle,
  ...
  SystemTable->BootServices->HandleProtocol(ImageHandle, &liGuid,
                                            (void **)&loaded_image);
  ...
  SystemTable->BootServices->AllocatePool(EfiLoaderData, infoSize,
                                          (void **)&fileInfo);
  ...
  SystemTable->BootServices->GetMemoryMap(&mapSize, NULL, &mapKey, &descSize,
                                          &descVer);
  mapSize += 2 * descSize; // a little extra in case it grows

  SystemTable->BootServices->AllocatePool(EfiLoaderData, mapSize,
                                          (void **)&memMap);
  SystemTable->BootServices->GetMemoryMap(&mapSize, memMap, &mapKey, &descSize,
                                          &descVer);
  ...
  if (!EFI_ERROR(status)) {
    ...
    SystemTable->BootServices->AllocatePool(EfiLoaderData, elfInfoSize,
                                            (void **)&elfInfo);
  ...
  if (!EFI_ERROR(status)) {
    ...
    SystemTable->BootServices->AllocatePool(EfiLoaderData, fontInfoSize,
                                            (void **)&fontInfo);
    ...
    EFI_PHYSICAL_ADDRESS font_addr =
        0x2000000; // 32 MB — safely above kernel and ELF
    SystemTable->BootServices->AllocatePages(AllocateAddress, EfiLoaderData,
                                             fontPages, &font_addr);
  ...
  // -----------------------------------------------------------------------
  // 3d. Setup High-Half Mapping Page Tables
  // -----------------------------------------------------------------------
  EFI_PHYSICAL_ADDRESS hh_pt_addr;
  SystemTable->BootServices->AllocatePages(AllocateAnyPages, EfiLoaderData, 8,
                                           &hh_pt_addr);
  uint64_t *new_pml4 = (uint64_t *)hh_pt_addr;
  uint64_t *dm_pdpt = (uint64_t *)(hh_pt_addr + 0x1000);
  uint64_t *dm_pd0 = (uint64_t *)(hh_pt_addr + 0x2000);
  uint64_t *dm_pd1 = (uint64_t *)(hh_pt_addr + 0x3000);
  uint64_t *dm_pd2 = (uint64_t *)(hh_pt_addr + 0x4000);
  uint64_t *dm_pd3 = (uint64_t *)(hh_pt_addr + 0x5000);
  uint64_t *k_pdpt = (uint64_t *)(hh_pt_addr + 0x6000);
  uint64_t *k_pd0 = (uint64_t *)(hh_pt_addr + 0x7000);

  for (int i = 0; i < 512 * 8; i++) {
    new_pml4[i] = 0;
  }

  // Direct Map at 0xFFFF800000000000 (PML4 256) -> 4GB
  new_pml4[256] = (uint64_t)dm_pdpt | 3;
  dm_pdpt[0] = (uint64_t)dm_pd0 | 3;
  dm_pdpt[1] = (uint64_t)dm_pd1 | 3;
  dm_pdpt[2] = (uint64_t)dm_pd2 | 3;
  dm_pdpt[3] = (uint64_t)dm_pd3 | 3;
  for (int i = 0; i < 4; i++) {
    uint64_t *pd = (uint64_t *)(hh_pt_addr + 0x2000 + i * 0x1000);
    for (int j = 0; j < 512; j++) {
      pd[j] = ((i * 512 + j) * 0x200000ULL) | 0x83; // Present, RW, Huge
    }
  }

  // Kernel Map at 0xFFFFFFFF80000000 (PML4 511) -> 1GB
  new_pml4[511] = (uint64_t)k_pdpt | 3;
  k_pdpt[510] = (uint64_t)k_pd0 | 3;
  for (int j = 0; j < 512; j++) {
    k_pd0[j] = (j * 0x200000ULL) | 0x83; // Present, RW, Huge
  }
  ...
  if (EFI_ERROR(status)) {
    ...
    SystemTable->BootServices->AllocatePool(EfiLoaderData, mapSize,
                                            (void **)&memMap);
    SystemTable->BootServices->GetMemoryMap(&mapSize, memMap, &mapKey,
                                            &descSize, &descVer);
  ...
  // -----------------------------------------------------------------------
  // 6. Setup High-Half Mapping & Jump to Rust kernel
  // -----------------------------------------------------------------------

  // Read current CR3 (UEFI's PML4)
  uint64_t cr3;
  __asm__ volatile("mov %%cr3, %0" : "=r"(cr3));
  uint64_t *old_pml4 = (uint64_t *)(cr3 & ~0xFFF);

  // Provide UEFI mappings in our new PML4 for seamless transition
  for (int i = 0; i < 512; i++) {
    if (!new_pml4[i]) {
  ...
  KernelEntry kStart = (KernelEntry)(kernel_addr + 0xFFFFFFFF80000000ULL);
  kStart(&bInfo);
